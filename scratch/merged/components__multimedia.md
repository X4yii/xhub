...
Para cargar medios desde URLs remotas, consulta `02-remote-assets.md`.
...
1. **FFmpegProcessDecoder** (principal): Genera un subproceso de FFmpeg, canaliza frames BGRA crudos mediante NIO.
2. **JCodecVideoDecoder** (respaldo): Decodificador H.264 puro Java usando JCodec. Se activa cuando FFmpeg no está disponible (Android, sistema de archivos no ejecutable, binario faltante).
...
- Los píxeles del frame se copian a un `DynamicTexture` mediante `System.arraycopy`
- Se renderiza mediante `Gui.drawModalRectWithCustomSizedTexture()`
...
For loading media from remote URLs, see `02-remote-assets.md`.
...
1. **FFmpegProcessDecoder** (primary): Spawns an FFmpeg subprocess, pipes raw BGRA frames via NIO.
2. **JCodecVideoDecoder** (fallback): Pure-Java H.264 decoder using JCodec. Activated when FFmpeg is unavailable (Android, noexec filesystem, missing binary).

The fallback is automatic and transparent.

### Rendering

- Each frame is polled from the decoder based on the master clock (`MediaPlayer.getPlaybackTimeSec()`)
- Frame pixels are copied to a `DynamicTexture` via `System.arraycopy`
- Rendered via `Gui.drawModalRectWithCustomSizedTexture()`
- Shows "Loading..." text while the decoder initializes
- Shows error message in red if the decoder fails
